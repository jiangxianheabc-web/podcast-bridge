#!/usr/bin/env python3
"""
2026-09-07 丽哥拍板: 凑齐 6 个"4 集日"到 5 集
策略: 直走 Groq audio_url 路径，每集 retry 3 次

6 集清单 (id, daily_date, podcast, title):
  1. id=8506   2026-08-24  屠龙之术                中国一级半市场水下江湖
  2. id=9001   2026-08-25  半拿铁 | 商业沉浮录       No.216 周鸿祎红衣大炮
  3. id=8911   2026-08-26  皮蛋漫游记              Vol.96 Mr.厉害
  4. id=9376   2026-08-28  不合时宜                基因编辑女童之死
  5. id=9706   2026-08-29  纵横四海                EP86 颠覆工作流
  6. id=10526  2026-09-01  晚点聊 LateTalk         180 具身智能金钱游戏

依据: 9-07 16:14 丽哥坚持要把 6 天补到 5 集, 推荐清单已拍板 (A 选项)
"""
import os
import sys
import time as _t
...
# 6 个目标 (id, daily_date) — 按 daily_date 升序
TARGET_IDS = [
    (8506, "2026-08-24"),
    (9001, "2026-08-25"),
    (8911, "2026-08-26"),
...
def main():
    print(f"🚀 Makeup backfill 启动 - {datetime.now().isoformat()}")
    ...
    if ok_count > 0:
        ...
        try:
            ...
            for month in ["2026-08", "2026-09"]:
                r = subprocess_run(
                    ["python3", str(SCRIPT_DIR / "regenerate_monthly_index.py"), "--month", month],
                    capture_output=True, text=True, cwd=str(SKILL_DIR)
                )
                print(f"   {month}: {r.stdout.strip().split(chr(10))[-1] if r.returncode == 0 else r.stderr[-200:]}")
